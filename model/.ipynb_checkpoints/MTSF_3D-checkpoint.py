...
#from .Decoders_MTSF_3D import Decoder5,Decoder4,Decoder3,Decoder2
from .Decoders_MTSF_3D_AC import Decoder5,Decoder4,Decoder3,Decoder2
...
class MTSM(nn.Module):
    ...
    def forward(self,x):
        ...
        hx = x[:,:,0,:,:] #只有预测的那张图片
        #stdge 3D
        y_1,y_2,y_3,y_4 = self.backbone[0](x)
       # print(y_1.size(),y_2.size(),y_3.size(),y_4.size())
        ...
        #hx1 = self.Channel_2D[0](hx1)
        d2 = self.stage2_1(y_1)

        #hx2 = self.Channel_2D[1](hx2)
        d3 = self.stage3_1(y_2)

        #hx3 = self.Channel_2D[2](hx3)
        d4 = self.stage4_1(y_3)

        #hx4 = self.Channel_2D[3](hx4)
        d5 = self.stage5_1(y_4)
