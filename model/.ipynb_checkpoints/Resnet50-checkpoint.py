...
class DWCONV(nn.Module):
    def __init__(self,in_ch=3,out_ch=3):
        ...
        self.conv_s1 = nn.Conv2d(in_ch,in_ch,kernel_size=3, stride=1, padding=1, groups=in_ch//4, bias=False)
...
#包含一个3d卷积和3d池化，将时间维变成1，然后去掉变成2d的，尺寸变成一半
class Tran3Dto2D(nn.Module):
    def __init__(self,in_ch=3,out_ch=3):
        super(Tran3Dto2D,self).__init__()
        self.conv_s1 = nn.Conv3d(in_ch,out_ch,kernel_size=(3, 3, 3), padding=(0, 1, 1))#(shape=(b,c,2,288,288))
        self.bn_s1 = nn.BatchNorm3d(out_ch)
        self.relu_s1 = nn.ReLU()
        self.pool1 = nn.MaxPool3d(kernel_size=(2, 2, 2), stride=(2, 2, 2))#(shape=(b,c,1,144,144))
    def forward(self,x):

        hx = x
        xout = self.relu_s1(self.bn_s1(self.conv_s1(hx)))
        xout = self.pool1(xout)
        xout = torch.squeeze(xout)
        return xout
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    model = ResNet(block, layers, **kwargs)
    if pretrained:
        #pretrained_dict = torch.load("/home/ailvin/下载/resnet18-5c106cde.pth")
        pretrained_dict = torch.load("/home/ailvin/下载/resnet50-19c8e357.pth")
